...
-- [ ALTER THESE VALUES ] ---- 
local printTime			= 60							--	How long the printer takes to generate money.
local minPrint 			= 2500						-- 	The minimum amount the printer can generate.
local maxPrint			= 4375							-- 	The maximum amount the printer can generate.
...
local coolantLoss 		= 3.3							--	Percentage of Coolant lost for each print.
------------------------------ 
...
function ENT:Use(activator)
	if ( activator:IsPlayer() and self:GetNWInt("PrintA") >= 1 ) then
		...
		DarkRP.notify(activator, 0, 4, "You have collected $" .. self:GetNWInt("PrintA").. " from a VIP Gold Printer." )
		self:SetNWInt("PrintA",0)
	end
end
