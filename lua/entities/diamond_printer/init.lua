...
-- [ ALTER THESE VALUES ] ---- 
local printTime			= 180							--	How long the printer takes to generate money.
local minPrint 			= 8000							-- 	The minimum amount the printer can generate.
local maxPrint			= 12000							-- 	The maximum amount the printer can generate.
...
local coolantLoss 		= 10							--	Percentage of Coolant lost for each print.
------------------------------ 
...
function ENT:Use(activator)
	if ( activator:IsPlayer() and self:GetNWInt("PrintA") >= 1 ) then
		...
		DarkRP.notify(activator, 0, 4, "You have collected $" .. self:GetNWInt("PrintA").. " from a Diamond Printer." )
		self:SetNWInt("PrintA",0)
	end
end
