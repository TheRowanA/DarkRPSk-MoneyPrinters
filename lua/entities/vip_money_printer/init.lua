...
local minPrint 			= 3000							-- 	The minimum amount the printer can generate.
local maxPrint			= 5250							-- 	The maximum amount the printer can generate.
...
function ENT:Use(activator)
	if ( activator:IsPlayer() and self:GetNWInt("PrintA") >= 1 ) then
		...
		DarkRP.notify(activator, 0, 4, "You have collected $" .. self:GetNWInt("PrintA").. " from a VIP Standard Printer." )
		self:SetNWInt("PrintA",0)
	end
end
