...
local minPrint 			= 3400							-- 	The minimum amount the printer can generate.
local maxPrint			= 5100							-- 	The maximum amount the printer can generate.
local upgradedExtra 	= maxPrint * 1.1				-- 	If the printer is upgraded, how much extra will they get?
local printerColor 		= Color( 204, 204, 0, 255 )	--  The color of the actual printer prop.
local coolantSystem 	= true							-- 	Toggles if the coolant system is enabled for this printer.
local coolantLoss 		= 6.6						--	Percentage of Coolant lost for each print.
------------------------------ 
...
function ENT:Use(activator)
	if ( activator:IsPlayer() and self:GetNWInt("PrintA") >= 1 ) then
		...
		DarkRP.notify(activator, 0, 4, "You have collected $" .. self:GetNWInt("PrintA").. " from a Gold Printer." )
		self:SetNWInt("PrintA",0)
	end
end
