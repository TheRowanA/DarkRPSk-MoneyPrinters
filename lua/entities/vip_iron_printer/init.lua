...
local minPrint 			= 5000							-- 	The minimum amount the printer can generate.
local maxPrint			= 8750							-- 	The maximum amount the printer can generate.
local upgradedExtra 	= maxPrint * 1.2				-- 	If the printer is upgraded, how much extra will they get?
local printerColor 		= Color( 192, 192, 192, 255 )	--  The color of the actual printer prop.
...
function ENT:Use(activator)
	if ( activator:IsPlayer() and self:GetNWInt("PrintA") >= 1 ) then
		...
		DarkRP.notify(activator, 0, 4, "You have collected $" .. self:GetNWInt("PrintA").. " from a VIP Iron Printer." )
		self:SetNWInt("PrintA",0)
	end
end
