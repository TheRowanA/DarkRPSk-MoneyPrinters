...
-- [ ALTER THESE VALUES FOR YOUR SERVER ] ---- 
local printTime			= 240							--	How long the printer takes to generate money.
local minPrint 			= 200							-- 	The minimum amount the printer can generate.
local maxPrint			= 300							-- 	The maximum amount the printer can generate.
local upgradedExtra 	= maxPrint * 1.1				-- 	If the printer is upgraded, how much extra will they get?
local printerColor 		= Color( 255, 255, 255, 255 )	--  The colour of the actual printer prop.
local coolantSystem 	= true							-- 	Toggles if the coolant system is enabled for this printer.
local coolantLoss 		= 13							--	Percentage of Coolant lost for each print.
------------------------------ 
...
function ENT:Use(activator)
	if ( activator:IsPlayer() and self:GetNWInt("PrintA") >= 1 ) then
		...
		DarkRP.notify(activator, 0, 4, "You have collected $" .. self:GetNWInt("PrintA").. " from a Standard Printer." )
		self:SetNWInt("PrintA",0)
	end
end
